param(
    ...
    [string]$SummaryPath = ".\all_candidates_summary.json"
)
...
foreach ($candidate in $candidates) {
    $jsonPath = Join-Path $outputFullPath ("candidate_{0}_results.json" -f $candidate.Id)
    $args = @(
        "--tier", $Tier,
        "--plaintext", $Plaintext,
...
$candidateSummaries = foreach ($candidate in $candidates) {
    ...
    [pscustomobject]@{
        ...
        states_checked = [UInt64]$parsed.states_checked
...
$totalCandidateChecks = ($candidateSummaries | Measure-Object -Property states_checked -Sum).Sum
$literalStates = if ($candidateSummaries.Count -gt 0) { @($candidateSummaries)[0].states_checked } else { 0 }
$summary = [pscustomobject]@{
    ...
    literal_states_checked_per_candidate = $literalStates
    aggregate_candidate_state_checks = $totalCandidateChecks
    literal_states_per_second_wall = $(if ($watch.Elapsed.TotalSeconds -gt 0) { $literalStates / $watch.Elapsed.TotalSeconds } else { 0 })
    candidate_state_checks_per_second_wall = $(if ($watch.Elapsed.TotalSeconds -gt 0) { $totalCandidateChecks / $watch.Elapsed.TotalSeconds } else { 0 })
